...
const QualityList: React.FC<QualityListProps> = ({
  ...
  const fetchPendingEntries = async () => {
    try {
      setLoading(true);
      const { data, error } = await getFabricEntries({ status: 'PENDING_QUALITY' });
  ...
  if (loading) {
    return (
      <div className="bg-white shadow rounded-lg p-6">
        <h2 className="text-lg font-medium text-gray-900 mb-4">Pending Quality Check</h2>
  ...
  return (
    <div className="bg-white shadow rounded-lg p-6">
      <h2 className="text-lg font-medium text-gray-900 mb-4">
        Pending Quality Check ({fabricEntries.length})
      </h2>
      ...
      {fabricEntries.length === 0 ? (
        <div className="text-center py-8">
          ...
          <h3 className="mt-2 text-sm font-medium text-gray-900">No entries pending quality check</h3>
          <p className="mt-1 text-sm text-gray-500">
            All fabric entries have been processed or none have been created yet.
          </p>
        </div>
      ) : (
